...
async function initDatabase() {
  ...
  if (OWNER_EMAIL && OWNER_PASSWORD) {
    const ownerRows = await query("SELECT id FROM users WHERE email = ?", [OWNER_EMAIL]);
    if (ownerRows.length === 0) {
      const passwordHash = await bcrypt.hash(OWNER_PASSWORD, 10);
      const id = newId("usr");
      const createdAt = new Date().toISOString();
      await run(
        "INSERT INTO users (id, email, name, password_hash, is_admin, deleted_at, created_at) VALUES (?, ?, ?, ?, 1, NULL, ?)",
        [id, OWNER_EMAIL, "Admin", passwordHash, createdAt]
      );
      await run("INSERT INTO user_data (user_id, data, updated_at) VALUES (?, ?, ?)", [
...
app.post("/api/auth/register", ah(async (req, res) => {
  ...
  await run(
    "INSERT INTO users (id, email, name, password_hash, is_admin, deleted_at, created_at) VALUES (?, ?, ?, ?, ?, NULL, ?)",
...
app.post("/api/auth/login", ah(async (req, res) => {
  ...
  if (user.deleted_at) {
    ...
    if (daysSince <= ACCOUNT_RECOVERY_DAYS) {
      await run("UPDATE users SET deleted_at = NULL WHERE id = ?", [user.id]);
      restored = true;
    } else {
      // Recovery window has passed -- the account is permanently gone. Clean up
      // and respond exactly as if no such account existed.
      await run("DELETE FROM feedback WHERE user_id = ?", [user.id]);
      await run("DELETE FROM user_data WHERE user_id = ?", [user.id]);
...
app.get("/api/me", requireAuth, ah(async (req, res) => {
  ...
  const dataRows = await query("SELECT data FROM user_data WHERE user_id = ?", [req.user.id]);
...
app.delete("/api/me", requireAuth, ah(async (req, res) => {
  ...
  await run("UPDATE users SET deleted_at = ? WHERE id = ?", [new Date().toISOString(), req.user.id]);
  res.json({ ok: true });
}));
